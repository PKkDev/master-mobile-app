...
namespace MasterMobile.Templates.PageState
{
        ...
        private async Task ChooseStateProperty(object newValue)
        {
            ...
            try
            {
                    ...
                    .Where(stateCondition => stateCondition.State != null && stateCondition.State.ToString().Equals(newValue.ToString())))
                {
                    if (Content != null)
                    {
                        //быстрая анимация скрытия
                        await Content.FadeTo(0, 100U);
                        //Полностью скрываем с экрана старое состояние
                        Content.IsVisible = false;
                        //Позволяем UI-потоку отработать свою очередь сообщений и гарантировано скрыть предыдущее состояние
                        await Task.Delay(30);
                    }
